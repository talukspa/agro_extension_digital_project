...
- **Four secrets must already exist in the project before the first `terragrunt plan`**, because the stack reads them with `run_cmd` while *evaluating* config: `webhook-verify-token`, `whatsapp-app-secret`, `wsp-token-aa`, `wsp-token-pp`. Create the last three with `scripts/local-deploy/05-create-app-secrets.sh <env>`. The AA and PP numbers hang off the same Meta app, so **one** App Secret validates both inbound endpoints; the outbound access tokens **are** per-number (a token for one returns 401 against the other's number).
